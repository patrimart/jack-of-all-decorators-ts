 ...
 * @returns {DIMap[]}
 */
export function getInjectablesByClass (clazz: IInjectable): DIMap[] {
 ...
 * @param {IInjectable} [clazz] - the optional Class to filter on.
 * @returns {DIMap[]}
 */
export function getInjectablesByModule (module: string, clazz?: IInjectable): DIMap[] {
    ...
    const di = DI_MAP.filter(
            di => di[1].every(
                (d, i) => d === "*" || d === modParts[i]
            )
        ).filter(
            di => clazz === undefined || di[0] === clazz.DI_CLASS_ID
        );
