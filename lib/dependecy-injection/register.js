...
function numActive(clazz, module) {
    try {
        if (clazz === undefined && module === undefined) {
            return module_1.DI_MAP.filter(function (d) { return !!d[2].instance; }).length;
        }
        if (clazz !== undefined && module === undefined) {
            return module_1.getInjectablesByClass(clazz).filter(function (d) { return !!d[2].instance; }).length;
        }
        return module_1.getInjectablesByModule(module, clazz).filter(function (d) { return !!d[2].instance; }).length;
    }
    catch (err) {
        return 0;
    }
}
exports.numActive = numActive;
